...
namespace SkiaSharp
{
	internal static class SkiaApi
	{
		...
		[DllImport(SKIA, CallingConvention = CallingConvention.Cdecl)]
		[return: MarshalAs(UnmanagedType.I1)]
		public extern static bool sk_bitmap_try_alloc_pixels_with_color_table(sk_bitmap_t cbitmap, ref SKImageInfo requestedInfo, sk_colortable_t ctable, uint flags);
		[DllImport(SKIA, CallingConvention = CallingConvention.Cdecl)]
		...
		[DllImport(SKIA, CallingConvention = CallingConvention.Cdecl)]
		public extern static sk_vertices_t sk_vertices_make_copy(SKVerticesVertexMode vmode, int vertexCount, [In] SKPoint[] positions, [In] SKPoint[] texs, [In] SKColor[] colors, int indexCount, [In] UInt16[] indices);
	}
}
